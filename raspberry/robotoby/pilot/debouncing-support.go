...
func (deb *debouncedValue) storeIfClean(value int) {
	bufferMax := application.State.Config.DebounceBufferSize
	...
	if len(deb.values) == 0 {
		...
		for index := 1; index < bufferMax; index++ {
		...
		if value <= maxVar*avg {
			...
			for index := bufferMax - 1; index > 0; index-- {
				deb.values[index] = deb.values[index-1]
			}
			deb.values[0] = value
		} else {
			application.Debug(fmt.Sprintf("Found var to debounce : current AVG = %v  VAR = %v for VALUE = %v", avg, maxVar, value))
		}
	}
}
